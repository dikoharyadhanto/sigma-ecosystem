...
exports.DIR_CLOSE_COVERAGE_CONFIG = exports.PLAN_EXEC_COVERAGE_CONFIG = exports.DIR_INTENT_COVERAGE_CONFIG = void 0;
exports.checkFidelityCoverage = checkFidelityCoverage;
function findAllIds(content, pattern) {
    const found = new Set();
...
function countTableRows(content, tableName) {
...
function countLedgerReferencesToTable(ledgerContent, tableName) {
    const escaped = tableName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const pattern = new RegExp(`${escaped}\\s*#\\d+`, 'g');
    const matches = ledgerContent.match(pattern);
    return matches ? matches.length : 0;
}
function checkFidelityCoverage(sourceContent, ledgerContent, config) {
    const gaps = [];
...
exports.DIR_INTENT_COVERAGE_CONFIG = {
...
exports.PLAN_EXEC_COVERAGE_CONFIG = {
...
exports.DIR_CLOSE_COVERAGE_CONFIG = {
    idPatterns: [],
    namedTables: [
...
//# sourceMappingURL=fidelityCoverage.js.map