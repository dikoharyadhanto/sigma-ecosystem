...
exports.getInactiveIntentWarnings = getInactiveIntentWarnings;
...
function getInactiveIntentWarnings(data) {
    const warnings = [];
    for (const iv of data.intent.versions) {
        if (iv.state !== 'INACTIVE')
            continue;
        const hanging = [];
        ...
        for (const pv of relatedPlans) {
            if (pv.state !== 'SUPERSEDED')
                hanging.push(`PLAN ${pv.version} (${pv.state})`);
        }
...
function collectIntentCascadeTargets(data, version) {
    const roadmap = data.roadmap.versions.filter(v => v.intent_version_ref === version && v.state !== 'SUPERSEDED');
    const plan = data.plan.versions.filter(v => v.intent_version_ref === version && v.state !== 'SUPERSEDED');
    const planVersionSet = new Set(plan.map(v => v.version));
    const exec = data.exec.versions.filter(v => !!v.plan_version_ref && planVersionSet.has(v.plan_version_ref) && v.state !== 'SUPERSEDED');
    const close = data.close.versions.filter(v => v.intent_version_ref === version && v.state !== 'SUPERSEDED');
...
function previewIntentSupersedeCascade(data, version) {
    return collectIntentCascadeTargets(data, version);
}
function supersedeIntentVersion(data, version, reason) {
    const now = new Date().toISOString();
    const target = data.intent.versions.find(v => v.version === version);
    if (!target)
        throw new Error(`INTENT version ${version} not found`);
    ...
    const successor = data.intent.versions.find(v => v.version !== version && (v.state === 'LOCKED' || v.state === 'DRAFT'));
    if (successor)
        target.superseded_by = successor.version;
    ...
    for (const rv of roadmap) {
        ...
        rv.updated_at = now;
        if (data.roadmap.active_version === rv.version)
            data.roadmap.active_state = 'SUPERSEDED';
    }
    for (const pv of plan) {
        pv.state = 'SUPERSEDED';
        pv.supersede_reason = cascadeReason;
        pv.updated_at = now;
        if (data.plan.active_version === pv.version)
    ...
    for (const ev of exec) {
        ...
        ev.updated_at = now;
        if (data.exec.active_version === ev.version)
    ...
    for (const cv of close) {
        ...
        cv.updated_at = now;
        if (data.close.active_version === cv.version)
            data.close.active_state = 'SUPERSEDED';
    }
}
// ── PLAN Mutations ────────────────────────────────────────────────────────────
...
// ── CLOSE Mutations ───────────────────────────────────────────────────────────
function registerCloseDraft(data, version, filePath, intentVersionRef) {
    ...
    const conflict = data.close.versions.find(v => v.intent_version_ref === intentVersionRef && v.state !== 'SUPERSEDED');
    if (conflict) {
        throw new Error(`DIR-CLOSE already exists for INTENT ${intentVersionRef} (${conflict.version}, ${conflict.state}). ` +
