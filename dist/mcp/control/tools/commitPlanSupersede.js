"use strict";
// Stage F (W2 batch, continued) — sigma_commit_plan_supersede. Mirrors
// commitIntentSupersede.ts for the FMN-PLAN domain.
Object.defineProperty(exports, "__esModule", { value: true });
exports.registerCommitPlanSupersedeTool = registerCommitPlanSupersedeTool;
...
function registerCommitPlanSupersedeTool(server) {
    server.registerTool('sigma_commit_plan_supersede', {
        title: 'Commit an approved plan supersede',
        description: 'Supersedes the FMN-PLAN version frozen by operation_ticket_id, auto-superseding any linked ' +
            'non-final DEV-EXEC — the MCP control-plane equivalent of `sigma plan supersede`. Requires a Director ' +
            'approval record for that exact ticket, recorded via the trusted local CLI (`sigma control approve ' +
            '<ticket_id>`). `reason` must match the text frozen at prepare time exactly. The approval is consumed ' +
            'on a successful commit and cannot be reused. FMN role only.',
        inputSchema: {
        ...
        return (0, shared_2.respondControlWrite)({
            tool: 'sigma_commit_plan_supersede',
            operationId: 'plan_supersede_commit',
            ...
            transactionFiles: (root) => [
                ...(0, planSupersedeService_1.planSupersedeTransactionFiles)(root),
            ...
            checkPreconditions: (root) => {
                ...
                const reasonHash = (0, shared_2.stableHash)({ reason: args.reason });
                ...
                if (ticket.operation_id !== 'plan_supersede') {
                    throw new errors_1.McpQueryError(contract_1.ERROR_CODES.INVALID_OPERATION, 'That ticket is not a plan_supersede ticket.');
                }
                ...
                if (ticket.arguments_hash !== reasonHash) {
                    throw new errors_1.McpQueryError(contract_1.ERROR_CODES.INVALID_OPERATION, 'The supplied `reason` does not match the text this ticket was prepared with.');
                }
                ...
                if (!ticket.target || !planEntry || planEntry.state === 'SUPERSEDED' || !planEntry.file) {
                ...
                if (!doc.present || doc.sha256 !== ticket.target.sha256) {
                    throw new errors_1.McpQueryError(contract_1.ERROR_CODES.STALE_ARTIFACT, 'The document has changed since this ticket was prepared.');
                }
                verifiedTargetVersion = ticket.target.version;
            },
        }, (root) => {
            const result = (0, planSupersedeService_1.supersedePlanUseCase)(root, verifiedTargetVersion, args.reason);
            const consumedAt = new Date().toISOString();
            (0, controlStore_1.markTicketConsumed)(root, args.operation_ticket_id, consumedAt);
            (0, controlStore_1.controlTestFailpoint)('plan_supersede_after_ticket_consumed');
            (0, controlStore_1.markApprovalConsumed)(root, args.approval_id, consumedAt);
            (0, controlStore_1.controlTestFailpoint)('plan_supersede_after_approval_consumed');
            return {
                chainVersion: result.chainVersion,
                version: result.version,
                cascadedExecs: result.cascadedExecs,
                operation_ticket_id: args.operation_ticket_id,
                approval_id: args.approval_id,
            };
        });
...
//# sourceMappingURL=commitPlanSupersede.js.map