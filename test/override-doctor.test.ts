...
import { setupTestEnv, runCli, makeProgressWithLockedIntent, TestEnv } from './helpers';
...
describe('sigma override survives sigma doctor while still in force', () => {
  ...
  it('keeps gate_2_open true after doctor when the overridden plan is still DRAFT', () => {
    ...
    const afterOverride = fs.readJsonSync(env.progressPath) as Record<string, any>;
    ...
    const afterDoctor = fs.readJsonSync(env.progressPath) as Record<string, any>;
  ...
  it('stops forcing the gate open once the overridden version is superseded by a real pivot', () => {
    ...
    const pivoted = fs.readJsonSync(env.progressPath) as Record<string, any>;
    ...
    fs.writeJsonSync(env.progressPath, pivoted, { spaces: 2 });
