...
**H2 Stage Section Rules:**

- `## Stage X.Y — Title` headings in ROADMAP must never be written manually — they are created automatically by `sigma plan new`.
- `sigma plan new` and `sigma plan promote` must always include both `--title` and `--focus` for official stage entries.
- Derived sections (Stage Overview, PLAN Breakdown) must never be manually edited — they are always regenerated by `sigma roadmap render`.
- Core Process Flow is manual. FMN should use it to capture the high-level product/system process in simple form, and `sigma roadmap render` must never overwrite it.
- FMN fills only the H3 body content (Focus, Main Output, Main Tasks, Dependency / Gate, Risk / Watch-Out) within each auto-created stage section.
