...
ROADMAP breaks a ratified DIR-INTENT into large build stages. Each Stage Overview row maps to an FMN-PLAN version. ROADMAP is partially CLI-managed — stage title/focus/status are supplied via `--title`/`--focus` on `sigma plan new` and `sigma plan promote`, stored in `progress-v<N>.json`, and rendered into the Stage Overview table by `sigma roadmap render`; the Core Process Flow and Planned Stage sections remain manual.

Planned Stage is distinct from Stage Overview: FMN fills it once at roadmap creation, referencing DIR-INTENT, to record the stage breakdown as initially planned (Stage/Title/Focus only — no Status/Reason). It is a static planning snapshot, never touched by `sigma roadmap render`; it may legitimately diverge from Stage Overview as actual implementation unfolds, and keeping it updated is optional.
...
*v0.11 (2026-09-18): ROADMAP gains a 4th section, Planned Stage (§ROADMAP, between Core Process Flow and Stage Overview) — a manual table (Stage/Title/Focus, no Status/Reason) that FMN fills once at roadmap creation by referencing DIR-INTENT, recording the stage breakdown as initially planned. Distinct from Stage Overview: it is a static snapshot, never touched by `sigma roadmap render`, and may legitimately diverge from actual implementation without being kept in sync. `PLANNED_STAGE` added to the roadmap doc-check's required section markers (structural presence only, not content). Director-directed template addition; no PLAN-EVAL.*
