...
ROADMAP breaks a locked DIR-INTENT into large build stages. Each ROADMAP H2 stage section maps to an FMN-PLAN version. ROADMAP is CLI-managed — stage sections are written by `sigma plan new` and `sigma plan promote`; content is rendered by `sigma roadmap render`.
