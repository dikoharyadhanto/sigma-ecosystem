...
| 3 | [PLAN-EVAL-03-LEGACY-PROGRESS-JSON-NAMING-AUDIT.md](PLAN-EVAL-03-LEGACY-PROGRESS-JSON-NAMING-AUDIT.md) | **Prioritas terendah, diflag langsung Director.** Istilah `progress.json` usang (arsitektur sudah pindah ke `progress-v<N>.json`) ditemukan di 46 file (`Sigma/` + `setup/`) — perlu audit klasifikasi genuine-drift vs historical-legit sebelum ada rencana perbaikan konkret. | Independen dari #1 dan #2 — sengaja ditaruh terakhir, dampaknya belum terbukti signifikan. |
