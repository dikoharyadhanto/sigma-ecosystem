...
**Tanggal**: 2026-08-12
**Status**: **Fase 1 dan Fase 2 SELESAI DIEKSEKUSI (2026-08-12)** — lihat §12 untuk urutan fase. Diimplementasikan: rename schema `LOCKED`→`RATIFIED` + `locked_at`→`ratified_at` khusus intent (§3.1–3.5); normalisasi baca legacy di `readChain()` dengan penanda `_migratedOnRead` (§3.2); migrasi persisten via `sigma doctor` dengan pelaporan `repaired[]` (§3.7, diverifikasi manual end-to-end); command `sigma intent lock` dihapus total, diganti `sigma intent ratify`, dengan tombstone yang mengarahkan ke command baru (§3.4, D-08); bump `SCHEMA_VERSION` → `1.1.0` (§3.3); taksonomi tier Sovereign/Operationalization ditambahkan ke `DIR-INTENT-TEMPLATE.md` §1.6/§6/§9 + judul §13.1 → "Ratify Requirement" (Fase 1, §4); sapuan terminologi menyeluruh — `SIGMA_PROTOCOL.md`, seluruh rule docs, registry, role-memory, README, 5 bridge stub root, dan 26 berkas `setup/targets/**` (§3.9). 257/257 test lulus (termasuk 5 test baru: 4 migrasi doctor + 1 tombstone). Bug nyata ditemukan & diperbaiki saat implementasi: `readExistingChain()` di `reconstruct.ts` melewati normalisasi `readChain()`, menyebabkan chain legacy gagal validasi diam-diam dan kehilangan riwayat title/focus PLAN — diperbaiki dengan mendelegasikan ke `readChain()` (lihat §3, catatan di kode).

**Belum dikerjakan**: Fase 3 (schema `amendments[]` + effective-state hash), Fase 4 (Section 14 template + docCheck optional-section), Fase 5 (command `sigma intent amendment`), dan sisa Fase 6 (dokumentasi mekanisme Amendment — sengaja ditunda sampai mekanismenya nyata, lihat §12). Menunggu arahan Director untuk melanjutkan.

**Temuan di luar scope, dicatat untuk sesi mendatang**: `Sigma/SIGMA-OPERATION-REGISTRY.json` punya drift struktural yang lebih luas dari sekadar RATIFIED — field `active_state`/`active_version` dipakai untuk domain (roadmap/close) yang skemanya sebenarnya cuma `state` tunggal, dan beberapa entry masih menyebut mekanisme `stale_intent`/`--ack-stale-intent` yang sudah tidak ada di kode sejak PLAN-EVAL-01. Tidak disentuh sesi ini — di luar scope rename RATIFIED, hanya bagian yang bersinggungan langsung (nilai `LOCKED`→`RATIFIED` untuk intent) yang diperbaiki. ~18 fixture test inline (`test/*.ts`) juga masih menulis literal `state: 'LOCKED'` untuk intent — tidak berdampak (dinormalisasi `readChain()` saat baca, seluruh suite tetap hijau) tapi belum disapu untuk konsistensi kosmetik.
