...
import { replaceSection } from './renderMarkers';
...
export function ensureAmendmentHistorySection(content: string): string {
  if (content.includes(SECTION_MARKER)) return content;
  const block = [
    '',
    '---',
    '',
...
export function renderAmendmentHistory(intentDocPath: string, chain: ChainState): void {
  if (!fs.existsSync(intentDocPath)) {
    throw new Error(`DIR-INTENT file not found: ${intentDocPath}`);
  }
  let content = fs.readFileSync(intentDocPath, 'utf8');
  content = ensureAmendmentHistorySection(content);
  content = replaceSection(content, 'amendment-history', generateAmendmentHistory(chain), 'DIR-INTENT file');
  fs.writeFileSync(intentDocPath, content, 'utf8');
}
