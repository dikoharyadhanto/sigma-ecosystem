...
export function computeGetEvidence(
  root: string | null,
  type: EvidenceType,
  version?: string
): unknown {
  ...
  const versions: ArtifactVersion[] = (type === 'plan' ? data.plan : data.exec).versions ?? [];
...
export function registerGetEvidenceTool(server: McpServer): void {
  server.registerTool(
    ...
    async ({ type, version }: { type: EvidenceType; version?: string }) =>
      respond('sigma_get_evidence', undefined, (root) => computeGetEvidence(root, type, version))
  );
}
