...
import { supersedePlanUseCase, planSupersedeTransactionFiles } from '../../../services/planSupersedeService';
...
export function registerCommitPlanSupersedeTool(server: McpServer): void {
  server.registerTool(
    'sigma_commit_plan_supersede',
    {
      title: 'Commit an approved plan supersede',
      description:
        'Supersedes the FMN-PLAN version frozen by operation_ticket_id, auto-superseding any linked ' +
      ...
      return respondControlWrite(
        {
          ...
          transactionFiles: (root) => [
            ...planSupersedeTransactionFiles(root),
          ...
          checkPreconditions: (root) => {
            ...
            if (ticket.operation_id !== 'plan_supersede') {
              throw new McpQueryError(ERROR_CODES.INVALID_OPERATION, 'That ticket is not a plan_supersede ticket.');
            }
            ...
            if (!ticket.target || !planEntry || planEntry.state === 'SUPERSEDED' || !planEntry.file) {
              throw new McpQueryError(ERROR_CODES.STALE_ARTIFACT, "The target FMN-PLAN no longer matches this ticket's target.");
            }
            const doc = readCanonicalArtifactFile(root, 'plan', planEntry.version, planEntry.file);
            if (!doc.present || doc.sha256 !== ticket.target.sha256) {
              throw new McpQueryError(ERROR_CODES.STALE_ARTIFACT, 'The document has changed since this ticket was prepared.');
            }
        ...
        (root) => {
          const result = supersedePlanUseCase(root, verifiedTargetVersion!, args.reason);
          const consumedAt = new Date().toISOString();
          markTicketConsumed(root, args.operation_ticket_id, consumedAt);
          controlTestFailpoint('plan_supersede_after_ticket_consumed');
          markApprovalConsumed(root, args.approval_id, consumedAt);
          controlTestFailpoint('plan_supersede_after_approval_consumed');
          return {
