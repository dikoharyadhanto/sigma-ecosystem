...
export function registerPrepareIntentRatifyTool(server: McpServer): void {
  server.registerTool(
    'sigma_prepare_intent_ratify',
    {
      title: 'Prepare an intent ratify operation ticket',
      description:
        'Freezes the active chain\'s current DRAFT intent (version, document hash, state_revision) into an ' +
        ...
        (root) => {
          ...
          if (chain.intent.state !== 'DRAFT') {
            throw new McpQueryError(
              ...
              `Active DIR-INTENT is "${chain.intent.state}", not DRAFT. Nothing to ratify.`
            );
          }
          ...
          if (!doc.present || !doc.sha256) {
            throw new McpQueryError(
              ERROR_CODES.INVALID_OPERATION,
              'The registered DRAFT file is not present on disk — nothing to ratify.'
            );
          }
