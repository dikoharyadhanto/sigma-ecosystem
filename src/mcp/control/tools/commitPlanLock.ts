...
import { lockPlanDraftUseCase, lockPlanDraftTransactionFiles } from '../../../services/planLockService';
...
export function registerCommitPlanLockTool(server: McpServer): void {
  server.registerTool(
      ...
      return respondControlWrite(
        {
          ...
          transactionFiles: (root) => [
            ...lockPlanDraftTransactionFiles(root),
          ...
          checkPreconditions: (root) => {
            ...
            if (ticket.operation_id !== 'plan_lock') {
              throw new McpQueryError(ERROR_CODES.INVALID_OPERATION, 'That ticket is not a plan_lock ticket.');
            }
        ...
        (root) => {
          const result = lockPlanDraftUseCase(root, verifiedTargetVersion!);
          const consumedAt = new Date().toISOString();
          markTicketConsumed(root, args.operation_ticket_id, consumedAt);
          controlTestFailpoint('plan_lock_after_ticket_consumed');
          markApprovalConsumed(root, args.approval_id, consumedAt);
          controlTestFailpoint('plan_lock_after_approval_consumed');
          return {
